...
class AssertPart implements ComposePart {
  ...
  compose(level: number, { bracket, quote }: ComposeConfig) {
    const fields = this.asserts
      .entrySeq()
      .map(([k, v]) => `${k}: ${quote(v)}`)
      .join(', ');
    const line = new ComposeLine(level, `assert ${bracket(fields)}`);
  ...
  composeWrap(level: number, config: ComposeConfig) {
    ...
    const { comma, quote } = config;
    const fields = this.asserts
      .entrySeq()
      .map(([k, v]) => `${k}: ${quote(v)}`)
      .toArray()
      .map((v, i, { length }) => (i + 1 < length ? `${v},` : v + comma))
